...
def main(argv):
    try:
        ...
        response = ec2Client.describe_instances(Filters=[{'Name': 'tag:Purpose', 'Values': ['Service Registry', ]},
...
def create_new_cluster(instance):
    ...
    # starting etcd
    write_line_to_file(target,
                       "nohup ./etcd --initial-advertise-peer-urls {0} --listen-peer-urls {0} --listen-client-urls {1} --advertise-client-urls {1} --initial-cluster-token service-discovery-client &".format(
                           peerurl, clienturl))
...
def sync_to_cluster(instance, serviceRegistryReservations):
    ...
    write_line_to_file(target,
                       'curl -s http://internal-Service-Registry-Internal-ELB-1537977493.us-east-1.elb.amazonaws.com:2379/v2/members -XPOST -H "Content-Type: application/json" -d \'{{"peerURLs":["{0}"]}}\''.format(
                           peerurl))
